...
describe('isCIDR', () => {
  test.each(ipConvertCases)('判断 $cidr 是否等于 $isCidr', ({ cidr, isCidr }) =>
    expect(IPv4.isCIDR(cidr as any)).toBe(isCidr)
  );
});

describe('isEqual', () => {
  test.each(ipConvertCases)('判断 $ip 是否等于 $long', ({ ip, long }) =>
    expect(IPv4.isEqual(ip, long)).toBe(true)
  );
  test.each(convertFailCases)('判断 $ip 是否为 false', ({ ip, ip2 }) =>
    expect(IPv4.isEqual(ip as any, ip2 as any)).toBe(false)
  );
});

describe('isValidIP', () => {
  test.each(ipConvertCases)('判断 $ip 是否等于 $long', ({ ip, strict }) =>
    expect(IPv4.isValidIP(ip, { strict })).toBe(true)
  );
  test.each(convertFailCases)('判断 $ip 是否为 false', ({ ip }) =>
    expect(IPv4.isValidIP(ip as any)).toBe(false)
  );
});

describe('ip2long', () => {
  expect(IPv4.ip2long('1:::1')).toBe(undefined);
...
describe('long2ip', () => {
  test.each(ipConvertCases)('将 $long 转换为 $ip', ({ ip, long }) =>
    expect(IPv4.long2ip(long as any)).toBe(ip)
  );
  test.each(convertFailCases)('判断 $ip 是否为 undefined', ({ ip }) =>
    expect(IPv4.long2ip(ip as any)).toBe(undefined)
  );
});

describe('toBinHex', () => {
  test.each(ipConvertCases)('判断 $ip 是否等于 $binHex', ({ ip, binHex }) =>
    expect(IPv4.toBinHex(ip)).toMatchObject(binHex)
  );
  test.each(convertFailCases)('判断 $ip 是否为 undefined', ({ ip }) =>
    expect(IPv4.toBinHex(ip as any)).toBe(undefined)
  );
});

describe('toIPv6Format', () => {
  test.each(ipConvertCases)('判断 $ip 是否等于 $v6format', ({ ip, v6format }) =>
    expect(IPv4.toIPv6Format(ip)).toMatchObject(v6format)
  );
  test.each(convertFailCases)('判断 $ip 是否为 undefined', ({ ip }) =>
    expect(IPv4.toIPv6Format(ip as any)).toBe(undefined)
  );
});
...
describe('toInverseMask', () => {
  test.each(subnetMaskCases)(
    '判断 $mask 是否等于 $inverse',
    ({ mask, inverse }) => expect(IPv4.toInverseMask(mask as any)).toBe(inverse)
  );
  test.each(subnetMaskCases)(
    '判断 $length 是否等于 $inverse',
    ({ length, inverse }) =>
...
describe('contains', () => {
  test.each(cidrConvertCases)(
    ...
    ({ cidr, contains }) =>
      expect(IPv4.contains(cidr as any, contains as any)).toBe(true)
  );
  test.each(convertFailCases)(
    '判断 $ip、$ip2 异常是否等于 false',
    ({ ip, ip2 }) => expect(IPv4.contains(ip as any, ip2 as any)).toBe(false)
  );
});
...
describe('parseCIDR', () => {
  test.each(cidrConvertCases)(
    '判断 $cidr 是否等于 $subnet',
    ({ cidr, subnet }) =>
      expect(IPv4.parseCIDR(cidr as any)).toMatchObject(subnet)
  );
  test.each(convertFailCases)('判断 $ip 异常是否等于 undefined', ({ ip }) =>
    expect(IPv4.parseCIDR(ip as any)).toBe(undefined)
  );
});

describe('parseSubnet', () => {
  test.each(cidrConvertCases)(
    '判断 $ip/$mask 是否等于 $subnet',
    ({ ip, mask, subnet }) =>
      expect(IPv4.parseSubnet(ip as any, mask as any)).toMatchObject(subnet)
  );
  test.each(convertFailCases)(
    '判断 $ip/$ip2 异常是否等于 undefined',
    ({ ip, ip2 }) =>
      expect(IPv4.parseSubnet(ip as any, ip2 as any)).toBe(undefined)
  );
});
...
describe('isConflict', () => {
  test.each(ipRangeCases)(
    '判断 $cidrs 是否存在冲突, $result',
    ({ cidrs, conflict }) => {
...
describe('isSameSubnet', () => {
  test.each(ipRangeCases)(
    '判断 $ip、$mask 是否等于在同一子网, $result',
    ({ ip, mask, result }) => {
      expect(IPv4.isSameSubnet(ip[0] as any, ip[1] as any, mask as any)).toBe(
        result
      );
    }
  );
  ...
  it('ipRange.fromLong 非法传参测试', () => {
    ...
    try {
      ...
      expect(error).toEqual(
        new Error(
          'Invalid range value, end must be greater than or equal to start'
        )
      );
    }
  });
